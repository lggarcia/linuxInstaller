...
##########################################
#DEBIAN TOOLS:Update bullseye to Bookworm#
##########################################

upgrade_to_bookworm()
{
    zenity --warning --title="DANGEROUS OPERATION" \
        --text="This will upgrade your system sources from 'bullseye' to 'bookworm'.\n\nEnsure you have backups. Do you want to proceed?" 2>/dev/null || return 1

    print_info "Starting upgrade process to Debian 12 (Bookworm)..."
    ...
    print_info "Updating APT sources from Bullseye to Bookworm..."
    sed -i 's/bullseye/bookworm/g' /etc/apt/sources.list
    find /etc/apt/sources.list.d -name "*.list" -exec sed -i 's/bullseye/bookworm/g' {} \;

    print_info "Fetching new Bookworm packages..."
    apt update
    ...
    zenity --info --text="Upgrade to Debian 12 (Bookworm) finished successfully.\n\nIt is highly recommended to reboot the system now." 2>/dev/null
}
